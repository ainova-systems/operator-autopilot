...
2. **Take the new image.** CI publishes and stops there — no workflow reaches into a running deployment. Every host, including this project's own instance, picks up `:latest` with `docker compose pull && docker compose up -d`. Automate that with one of:
   - **Watchtower**, the opt-in `watchtower` compose profile — watches the registry and recreates the containers when `:latest` changes: `docker compose -f deployment/docker-compose.yml --profile watchtower up -d`. The low-effort choice for an always-on host.
   ...
   - `deployment/deploy.sh` with a pinned `:<sha>`, driven by whatever change process you already have — deterministic and rollbackable in one command.
   - Pin a digest/tag and bump it through your normal deploy pipeline if you prefer reviewed updates over auto-pull.
...
> The same drift applies to any CI runner image that bakes the agent CLIs in at build time. Give its build the same `schedule:` + `no-cache` treatment, and refresh the runners — ephemeral runners re-pull `:latest` per job; long-lived runner containers need the Watchtower profile or a scheduled `pull && up -d`.
