...
- **`@operator/core`:** exports types/interfaces, Zod schemas, and error classes (schemas + errors are the only permitted runtime); imports only `zod` at runtime; never imports `adapters`/`engine`/`app`. Platform vocabulary (`PullRequest`/`Issue`/`MergeRequest`) is a blocker here — use `CodeReview`/`WorkItem`.
